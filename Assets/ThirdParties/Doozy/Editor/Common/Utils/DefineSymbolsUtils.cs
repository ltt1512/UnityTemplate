    ...
    public static class DefineSymbolsUtils
	{
		...
		public static void AddGlobalDefine(string id)
		{
			...
			foreach (BuildTargetGroup buildTargetGroup in (BuildTargetGroup[]) Enum.GetValues(typeof(BuildTargetGroup)))
                ...
                {
					var nameTarget = UnityEditor.Build.NamedBuildTarget.FromBuildTargetGroup(buildTargetGroup);
                    string defineSymbolsForGroup = PlayerSettings.GetScriptingDefineSymbols(nameTarget);
					...
					PlayerSettings.SetScriptingDefineSymbols(nameTarget, defines);
				}
		...
		public static void RemoveGlobalDefine(string id)
		{
			...
			foreach (BuildTargetGroup buildTargetGroup in (BuildTargetGroup[]) Enum.GetValues(typeof(BuildTargetGroup)))
				...
				{
                    var nameTarget = UnityEditor.Build.NamedBuildTarget.FromBuildTargetGroup(buildTargetGroup);
                    string[] array = PlayerSettings.GetScriptingDefineSymbols(nameTarget).Split(';');
					...
					PlayerSettings.SetScriptingDefineSymbols(nameTarget, StringBuilderContainer.ToString());
				}
		...
		public static bool HasGlobalDefine(string id, BuildTargetGroup? buildTargetGroup = null)
		{
			...
			string getDefineSymbols(BuildTargetGroup buildTargetGruop)
			{
                var nameTarget = UnityEditor.Build.NamedBuildTarget.FromBuildTargetGroup(buildTargetGruop);
				var symbols = PlayerSettings.GetScriptingDefineSymbols(nameTarget);
				return symbols;

            }
            return buildTargetGroupArray.Where(IsValidBuildTargetGroup).Any(buildTargetGroup1 => Array.IndexOf(getDefineSymbols(buildTargetGroup1).Split(';'), id) != -1);
		}
